			...
			foreach (KeyValuePair<string, IApplicationConfiguration> applicationItem in _parameters.ApplicationsConfiguration)
			{
				...
				foreach (KeyValuePair<string, ITargetConfiguration> targetItem in targetsForApplication)
				{
					...
					foreach (KeyValuePair<string, IPlatformConfiguration> platformItem in platformsForTarget)
					{
						...
						if (!_parameters.PlatformsConfiguration.ContainsKey(platformItem.Key))
						{
							_parameters.Context.CakeContext.LogAndThrow($"Platform with name {platformItem.Key} does not exists");
						}
						
						IConfiguration platformLevel = _parameters.PlatformsConfiguration[platformItem.Key];

						if (!_parameters.TargetsConfiguration.ContainsKey(targetItem.Key))
						{
							_parameters.Context.CakeContext.LogAndThrow($"Target with name {targetItem.Key} does not exists");
						}
						...
						else //at least validate if the platform defined in the target configuration really exists
						{
